...
def initialize_abstractive_model(sum_model, use_hf_pipeline=True, *args, **kwargs):
    ...
    if use_hf_pipeline:
        ...
        SUMMARIZER = pipeline("summarization", model=sum_model, tokenizer=sum_model, framework="pt")
    else:
        if sum_model == "bart":
            import bart_sum
...
def generic_abstractive(
    ...
    **kwargs
    ...
    # Truncate input for models like BART (max 1024 tokens)
    MAX_INPUT_TOKENS = 1024
    if not hf_inference_api and hasattr(summarizer, "tokenizer"):
        tokenizer = summarizer.tokenizer
        inputs = tokenizer(to_summarize, return_tensors="pt", truncation=True, max_length=MAX_INPUT_TOKENS)
        to_summarize = tokenizer.decode(inputs["input_ids"][0], skip_special_tokens=True)
    ...
    if isinstance(LECTURE_SUMMARIZED, list):
